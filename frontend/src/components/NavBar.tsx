  ...
  return (
    <nav className={`flex justify-center gap-5 ${darkMode?.state && "bg-slate-800"}`}>
      <NavLink
        className={`m-3 p-4 text-xl ${
          darkMode?.state
            ...
            : " bg-blue-400 hover:bg-blue-500 rounded-md font-medium text-white"
        }`}
        to={"/"}
      >
        All Entries
      ...
      <div className="flex flex-col">
        <button
          className={`my-auto rounded-full border-2 p-1 ${!darkMode?.state && "bg-indigo-300"}`}
      ...
      <div className="flex flex-col">
        <button
          className={`my-auto rounded-full border-2 p-1 ${darkMode?.state && "bg-indigo-600"}`}
