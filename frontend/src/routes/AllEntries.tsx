...
export default function AllEntries(){
    ...
    if(entries.length == 0){
        return(
            <section className={darkMode?.state ? "bg-slate-800 text-white" : ""}>
    ...
    return(
        <section className={`grid grid-cols-2 md:grid-cols-4 ${darkMode?.state && "bg-slate-800"}`}>
            {entries.map((entry: Entry, index: number) => {
                return(
                    <div id={entry.id} key={index} className={`bg-gray-300 shadow-md shadow-gray-500 m-3 p-4 rounded flex flex-col justify-between ${darkMode?.state 
                    && "bg-slate-700 text-white shadow-slate-900"}`}>
                        ...
                        <span className='text-sm md:text-lg space-x-3 flex'>
                            <p>Due for:</p>
